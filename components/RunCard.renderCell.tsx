...
export function renderCell<T extends ISimpleTableCell>(
  ...
  // ROW AGE
  const isAge = ((item) => item.isAge) as (
    item: any,
  ) => item is { name: string; treeItem: ITreeItem<ResultOrRuleOrMore> };
  if (isAge(data)) {
    const age = data;
            ...
            <div className="swcRowRule">
              ...
              {tryOr(() =>
                rule.relationships.map((rel, i) => {
                  const taxon =
                    rule.run.taxonomies[rel.target.toolComponent.index].taxa[rel.target.index];
                  return (
                    <Fragment key={rel.target.id}>
                      {i > 0 ? ',' : ''} {tryLink(() => taxon.helpUri, taxon.name)}
                    </Fragment>
                  );
                }),
              )}
  ...
  if (isResult(data)) {
    ...
    return columnIndex === 0
      ? // ExpandableTreeCell (td div.bolt-table-cell-content.flex-row.flex-center TreeExpand children)
        ...
        // calls TableCell
        ExpandableTreeCell({
          // As close to Table#TwoLineTableCell (which calls TableCell) as possible.
          children: (
            <>
              <Status
                {...status}
                className="bolt-table-two-line-cell-icon flex-noshrink bolt-table-status-icon"
                size={StatusSize.m}
                ariaLabel={result.level || 'warning'}
              />
      ...
      : TableCell({
          // Don't want SimpleTableCell as it has flex row.
          children: (() => {
            const rule = result._rule;
            switch (treeColumn.id) {
              case 'Details':
                const messageFromRule =
                  result._rule?.messageStrings?.[result.message.id ?? -1] ?? result.message;
                const formattedMessage =
                  format(messageFromRule.text || result.message?.text, result.message.arguments) ??
                  '';
                const formattedMarkdown = format(
                  messageFromRule.markdown || result.message?.markdown,
              ...
              case 'Baseline':
                return (
                  <Hi>{(result.baselineState && capitalize(result.baselineState)) || 'New'}</Hi>
                );
              case 'Level':
                return <Hi>{result.level || 'New'}</Hi>;
              case 'Bug':
                return tryOr(() => (
                  <Link href={result.workItemUris[0]} target="_blank">
                    <Icon
  ...
  if (isMore(data)) {
    return columnIndex === 0
      ? ExpandableTreeCell({
          children: (
            <Link onClick={data.onClick} tabIndex={-1}>
              Show All
            </Link>
          ),
          colspan,
  ...
  return message.match(rxLink)
    ? message.split(/(\[[^\]]*\]\([^\)]+\))/g).map((item, i) => {
        ...
        const href = (() => {
          ...
          const physicalLocation = result.relatedLocations?.find(
