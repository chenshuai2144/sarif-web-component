...
import { MobxFilter, recommendedDefaultState } from './FilterBar';
...
export class Viewer extends Component<ViewerProps> {
  ...
  private runStores = computedFn(
    (logs) => {
      ...
      const runStores = runs.map(
        (run, i) => new RunStore(run, i, filter, groupByAge, hideBaseline, showAge),
      );
  ...
  @computed get runStoresSorted() {
    ...
    return this.runStores(logs)
      .slice()
      .sorted((a, b) => b.filteredCount - a.filteredCount); // Highest count first.
  }
  ...
  render() {
    ...
    const nearElement = (() => {
      ...
      const filteredResultsCount = runStoresSorted.reduce(
        (total, run) => total + run.filteredCount,
        0,
      );
      if (filteredResultsCount === 0) {
        ...
        const showSuccess =
          successMessage &&
          (!startingFilterStateLevel.includes('error') ||
            currentfilterStateLevel.includes('error')) &&
          (!startingFilterStateLevel.includes('warning') ||
            currentfilterStateLevel.includes('warning'));
        ...
        <SurfaceContext.Provider value={{ background: SurfaceBackground.neutral }}>
          <Page>
            {this.warnOldVersion && (
              ...
              >
                Pre-SARIF-2.1 logs have been omitted. Use the Artifacts explorer to access all
                files.
              </MessageCard>
            )}
            {nearElement}
