...
export function renderPathCell(result: Result) {
  ...
  const resArtLoc = ploc?.artifactLocation ?? result.analysisTarget;
  ...
  const [path, fileName] = (() => {
    ...
    return index >= 0 ? [uri.slice(0, index), uri.slice(index + 1)] : [uri];
  })();
    ...
    () => (
      <div className="flex-column scroll-hidden">
        <div className={rowClasses}>
          <div className="fontsize font-size swcWidth100">
            {/* Constraining width to 100% to play well with the Tooltip. */}
            <Tooltip overflowOnly={true}>
              <pre style={{ margin: 0 }}>
                <code>
                  <Hi>{result.locations[0].logicalLocations[0].fullyQualifiedName}</Hi>
                </code>
              </pre>
            </Tooltip>
          </div>
    ...
    () => (
      <div className="flex-row scroll-hidden">
        ...
        <TooltipSpan text={href ?? uri}>
          {tryLink(getHref, uriWithEllipsis, 'swcColorUnset', onClick)}
        </TooltipSpan>
      </div>
    ),
  );
