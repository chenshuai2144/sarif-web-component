...
export class FilterBar extends React.Component<{
  ...
  render() {
    ...
    return (
      <AzFilterBar filter={filter}>
        <KeywordFilterBarItem filterItemKey="Keywords" placeholder="输入文字筛选" />
        {!hideBaseline && (
          <AzDropdownFilterBarItem
            filterItemKey="Baseline"
            placeholder="Baseline"
            showPlaceholderAsLabel
            items={['New', 'Unchanged', 'Updated', 'Absent'].map((text) => ({
              id: text.toLowerCase(),
              text,
            }))}
            selection={this.ms1}
        ...
        {!hideLevel && (
          <AzDropdownFilterBarItem
            ...
            placeholder="Level"
            showPlaceholderAsLabel
            items={['Error', 'Warning', 'Note', 'None'].map((text) => ({
              id: text.toLowerCase(),
              text,
            }))}
            selection={this.ms2}
